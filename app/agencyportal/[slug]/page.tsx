...
import { getAgencyByOwnerEmail, getAgencyById, createAgency, Agency } from "../../../lib/agency/agency";
...
export default function AgencyPage() {
  ...
  // If no agency: redirect to register page
  if (!agency) {
    return (
      <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center px-4">
        <div className="rounded-2xl border border-white/6 bg-slate-900/80 px-8 py-10 text-center shadow-lg backdrop-blur-xl">
          <p className="text-lg font-semibold">No Agency Found</p>
          <p className="mt-2 text-slate-400">You have not created an agency yet.</p>
          <div className="mt-6">
             <button onClick={() => window.location.href = '/agencyportal/register'} className="inline-flex items-center gap-3 rounded-full bg-[#D4AF37] px-5 py-3 text-sm font-semibold text-black">
                Create an Agency
             </button>
          </div>
        </div>
  ...
  // approved
  return <AgencyDashboard agency={agency} onAgencyUpdated={refreshAgency} />;
}
